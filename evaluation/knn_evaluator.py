...
class KNNEvaluator(BaseEvaluator):
    ...
    def add_commandline_args(parser):
        parser.add_argument(
            "--nb-knn", type=int, default=5, help="Number of NN to use."
        )
        return parser
    ...
    def prepare_evaluation(self):
        ...
        transform = TestTransform(self.image_size)
        if "lsun" in self.train_dataset or "lsun" in self.test_dataset:
            self._is_possible = False
            return
        self.trainset = ImageFolder(self.train_dataset, transform, True)
        self.valset = ImageFolder(self.test_dataset, transform, True)
    ...
    def evaluate(self, model, dataset=None, batch_size=50, step=None):
        ...
        val_loader = DataLoader(
            self.valset, batch_size=batch_size,
            num_workers=4, pin_memory=True
        )
